-- Migration: 011_contribution_scheduling.sql
...
-- =====================================================
-- CONTRIBUTIONS (Individual Payment Records)
-- Each member's payment for each round
-- =====================================================
CREATE TABLE IF NOT EXISTS contributions (
    ...
    status TEXT NOT NULL DEFAULT 'pending' CHECK (status IN (
        ...
        'completed',    -- Paid on time or within grace
        'late',         -- Paid late (after grace period)
        'defaulted',    -- Not paid after default threshold
        'waived',       -- Waived by admin
        'covered'       -- Covered by security deposit or guarantor
    )),
    ...
    total_charged DECIMAL(15, 2),  -- amount + late_fee
    ...
    -- Default handling
    default_recorded BOOLEAN DEFAULT false,
    security_deposit_applied DECIMAL(15, 2) DEFAULT 0,
    guarantor_covered BOOLEAN DEFAULT false,
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
        ...
        ALTER TABLE circles ADD COLUMN contribution_frequency TEXT DEFAULT 'monthly'
            CHECK (contribution_frequency IN ('daily', 'weekly', 'biweekly', 'monthly', 'quarterly'));
    END IF;
...
CREATE INDEX IF NOT EXISTS idx_contributions_circle ON contributions(circle_id);
...
-- Contributions - users see own, admins see all in circle
...
DROP POLICY IF EXISTS "Circle admins can view all contributions" ON contributions;
CREATE POLICY "Circle admins can view all contributions" ON contributions
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM circle_members
            WHERE circle_id = contributions.circle_id
            AND user_id = auth.uid()
...
-- Trigger to update circle stats when contribution is processed
...
BEGIN
    IF NEW.status IN ('completed', 'late') AND OLD.status = 'pending' THEN
        ...
        UPDATE circles SET
            total_collected = total_collected + COALESCE(NEW.total_charged, NEW.amount),
            total_late_payments = total_late_payments + CASE WHEN NEW.is_late THEN 1 ELSE 0 END,
            total_late_fees_collected = total_late_fees_collected + COALESCE(NEW.late_fee, 0),
            updated_at = NOW()
        ...
        UPDATE circle_members SET
            ...
            total_amount_paid = total_amount_paid + COALESCE(NEW.total_charged, NEW.amount),
            total_late_payments = total_late_payments + CASE WHEN NEW.is_late THEN 1 ELSE 0 END,
            total_late_fees_paid = total_late_fees_paid + COALESCE(NEW.late_fee, 0),
            updated_at = NOW()
        WHERE id = NEW.member_id;

        -- Update schedule stats
        UPDATE contribution_schedules SET
            received_contributions = received_contributions + 1,
            total_collected = total_collected + COALESCE(NEW.total_charged, NEW.amount),
            total_late_fees = total_late_fees + COALESCE(NEW.late_fee, 0),
            updated_at = NOW()
        WHERE id = NEW.schedule_id;
    END IF;

    IF NEW.status = 'defaulted' AND OLD.status != 'defaulted' THEN
        -- Update circle default count
        UPDATE circles SET
...
DROP TRIGGER IF EXISTS trigger_contributions_updated_at ON contributions;
...
COMMENT ON TABLE contributions IS 'Individual member contribution payments for each round';
