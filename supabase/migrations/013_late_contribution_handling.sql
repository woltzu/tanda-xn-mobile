-- Migration: 013_late_contribution_handling.sql
-- Description: Late Contribution Handling - Trust meets reality
...
-- =====================================================
CREATE TABLE IF NOT EXISTS auto_retry_config (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
-- =====================================================
CREATE TABLE IF NOT EXISTS auto_retry_history (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
-- =====================================================
CREATE TABLE IF NOT EXISTS late_contribution_events (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
-- =====================================================
CREATE TABLE IF NOT EXISTS redistribution_responses (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
-- INDEXES
-- =====================================================
CREATE INDEX IF NOT EXISTS idx_late_contrib_user ON late_contributions(user_id);
CREATE INDEX IF NOT EXISTS idx_late_contrib_cycle ON late_contributions(cycle_id);
CREATE INDEX IF NOT EXISTS idx_late_contrib_circle ON late_contributions(circle_id);
CREATE INDEX IF NOT EXISTS idx_late_contrib_status ON late_contributions(late_status);
CREATE INDEX IF NOT EXISTS idx_late_contrib_unresolved ON late_contributions(late_status)
    WHERE late_status NOT IN ('paid_late', 'partially_paid', 'covered', 'forgiven');
CREATE INDEX IF NOT EXISTS idx_late_contrib_grace_end ON late_contributions(grace_period_end)
...
CREATE INDEX IF NOT EXISTS idx_payment_plans_late ON payment_plans(late_contribution_id);
...
CREATE INDEX IF NOT EXISTS idx_retry_history_late ON auto_retry_history(late_contribution_id);
CREATE INDEX IF NOT EXISTS idx_retry_history_user ON auto_retry_history(user_id);

CREATE INDEX IF NOT EXISTS idx_late_events_late ON late_contribution_events(late_contribution_id);
CREATE INDEX IF NOT EXISTS idx_late_events_type ON late_contribution_events(event_type);

CREATE INDEX IF NOT EXISTS idx_redist_cycle ON redistribution_requests(cycle_id);
CREATE INDEX IF NOT EXISTS idx_redist_status ON redistribution_requests(request_status);

CREATE INDEX IF NOT EXISTS idx_restrictions_user ON user_restrictions(user_id);
CREATE INDEX IF NOT EXISTS idx_restrictions_active ON user_restrictions(user_id, restriction_type)
...
CREATE POLICY "late_contrib_select" ON late_contributions
    FOR SELECT USING (
        user_id = auth.uid() OR
...
CREATE POLICY "payment_plans_insert" ON payment_plans
    FOR INSERT WITH CHECK (user_id = auth.uid());

-- Installments - user sees own plans
CREATE POLICY "installments_select" ON payment_plan_installments
    FOR SELECT USING (
...
CREATE POLICY "retry_config_select" ON auto_retry_config
    FOR SELECT USING (
...
CREATE POLICY "retry_history_select" ON auto_retry_history
    FOR SELECT USING (user_id = auth.uid());

-- Late events - user sees own, leaders see circle
CREATE POLICY "late_events_select" ON late_contribution_events
    FOR SELECT USING (
        user_id = auth.uid() OR
...
CREATE POLICY "redist_select" ON redistribution_requests
    FOR SELECT USING (
...
CREATE POLICY "redist_resp_select" ON redistribution_responses
    FOR SELECT USING (user_id = auth.uid());

CREATE POLICY "redist_resp_update" ON redistribution_responses
...
SELECT
    ...
    p.full_name as user_name,
    c.name as circle_name,
    (SELECT COUNT(*) FROM payment_plan_installments WHERE payment_plan_id = pp.id AND installment_status = 'paid') as paid_count,
    (SELECT MIN(due_date) FROM payment_plan_installments WHERE payment_plan_id = pp.id AND installment_status = 'pending') as next_due_date,
