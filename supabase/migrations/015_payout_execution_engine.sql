-- Migration: 015_payout_execution_engine.sql
...
BEGIN
    ...
    LEFT JOIN (
        SELECT
            wallet_id,
            SUM(current_balance_cents) as total_savings,
    ...
    LEFT JOIN (
        SELECT
            wallet_id,
            SUM(amount_cents) as upcoming_total
...
LEFT JOIN (
    SELECT
        wallet_id,
        SUM(current_balance_cents) as savings_total,
...
SELECT
    ...
    SUM((pe.distribution->>'to_wallet')::BIGINT) / 100.0 as amount_to_wallets,
    SUM(COALESCE((pe.distribution->>'to_bank')::JSONB->>'amount_cents', '0')::BIGINT) / 100.0 as amount_to_banks,
    COUNT(*) FILTER (WHERE pe.distribution->>'to_bank' IS NULL) as wallet_only_payouts,
    COUNT(*) FILTER (WHERE pe.distribution->>'to_bank' IS NOT NULL) as external_payouts,
    ROUND(
        COUNT(*) FILTER (WHERE pe.distribution->>'to_bank' IS NULL)::DECIMAL /
