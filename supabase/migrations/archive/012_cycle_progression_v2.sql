-- Migration: 012_cycle_progression_v2.sql
-- Description: Cycle Progression Engine - Fixed for existing schema
...
DROP INDEX IF EXISTS idx_cc_circle;
DROP INDEX IF EXISTS idx_cc_status;
DROP INDEX IF EXISTS idx_cc_deadline;
DROP INDEX IF EXISTS idx_cc_recipient;
DROP INDEX IF EXISTS idx_contrib_cycle;
DROP INDEX IF EXISTS idx_contrib_user;
DROP INDEX IF EXISTS idx_contrib_status;
DROP INDEX IF EXISTS idx_events_cycle;
DROP INDEX IF EXISTS idx_events_type;
DROP INDEX IF EXISTS idx_def_user;
DROP INDEX IF EXISTS idx_def_circle;
DROP INDEX IF EXISTS idx_def_status;
DROP INDEX IF EXISTS idx_sched_user;
DROP INDEX IF EXISTS idx_sched_for;
DROP INDEX IF EXISTS idx_runs_started;
DROP INDEX IF EXISTS idx_xn_user;
DROP INDEX IF EXISTS idx_vouch_er;
DROP INDEX IF EXISTS idx_vouch_ee;
DROP INDEX IF EXISTS idx_pay_user;
DROP INDEX IF EXISTS idx_cron_type;
DROP INDEX IF EXISTS idx_alert_status;
DROP INDEX IF EXISTS idx_reserve_tx;
...
DROP FUNCTION IF EXISTS update_cycle_ts CASCADE;
...
CREATE TABLE vouches (
    ...
    vouch_status TEXT NOT NULL DEFAULT 'active',
...
CREATE TABLE circle_cycles (
    ...
    cycle_status TEXT NOT NULL DEFAULT 'scheduled',
...
CREATE TABLE scheduled_notifications (
    ...
    notification_status TEXT NOT NULL DEFAULT 'scheduled',
    sent_at TIMESTAMPTZ,
...
CREATE INDEX idx_cc_status ON circle_cycles(cycle_status);
...
CREATE INDEX idx_contrib_status ON cycle_contributions(contribution_status);
...
CREATE INDEX idx_def_status ON member_defaults(default_status);
...
CREATE INDEX idx_alert_status ON ops_alerts(alert_status);
...
-- Simple policies that don't rely on admin_users.status
...
CREATE POLICY "contrib_select" ON cycle_contributions FOR SELECT USING (
    user_id = auth.uid() OR EXISTS (SELECT 1 FROM circle_members WHERE circle_id = cycle_contributions.circle_id AND user_id = auth.uid())
);
...
CREATE POLICY "runs_select" ON cycle_engine_runs FOR SELECT USING (auth.uid() IS NOT NULL);
...
CREATE POLICY "reserve_tx_select" ON reserve_fund_transactions FOR SELECT USING (auth.uid() IS NOT NULL);
...
CREATE POLICY "cron_select" ON cron_job_logs FOR SELECT USING (auth.uid() IS NOT NULL);
CREATE POLICY "alert_select" ON ops_alerts FOR SELECT USING (auth.uid() IS NOT NULL);
...
BEGIN
    UPDATE circle_cycles SET
        collected_amount = (SELECT COALESCE(SUM(contributed_amount),0) FROM cycle_contributions WHERE cycle_id=NEW.cycle_id AND contribution_status IN ('completed','covered')),
        received_contributions = (SELECT COUNT(*) FROM cycle_contributions WHERE cycle_id=NEW.cycle_id AND contribution_status IN ('completed','covered')),
...
SELECT c.id as circle_id, c.name as circle_name, c.community_id, cc.id as cycle_id, cc.cycle_number, cc.cycle_status,
    ...
    CASE WHEN cc.cycle_status='collecting' AND cc.contribution_deadline<CURRENT_DATE THEN 'overdue'
         WHEN cc.cycle_status='grace_period' AND cc.grace_period_end<CURRENT_DATE THEN 'grace_expired'
         WHEN cc.cycle_status='payout_pending' AND cc.status_changed_at<NOW()-INTERVAL '3 days' THEN 'payout_stuck'
...
WHERE cc.cycle_status NOT IN ('closed','cancelled','skipped');

CREATE VIEW v_cycles_needing_attention AS
SELECT * FROM v_active_cycles WHERE attention_status!='normal' OR cycle_status IN ('payout_failed','payout_retry')
ORDER BY CASE cycle_status WHEN 'payout_failed' THEN 1 WHEN 'grace_period' THEN 2 WHEN 'deadline_reached' THEN 3 ELSE 4 END;
