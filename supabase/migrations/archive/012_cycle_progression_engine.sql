-- Migration: 012_cycle_progression_engine.sql
...
-- =====================================================
-- CIRCLE CYCLES TABLE
-- One record per cycle in a circle
-- =====================================================
CREATE TABLE IF NOT EXISTS circle_cycles (
    ...
    status TEXT NOT NULL DEFAULT 'scheduled' CHECK (status IN (
        'scheduled',          -- Cycle exists but not started
        'collecting',         -- Accepting contributions
        'deadline_reached',   -- Deadline passed, checking status
        'grace_period',       -- Waiting for late payments
        'ready_payout',       -- All conditions met for payout
        'payout_pending',     -- Payment processor transfer in progress
        'payout_completed',   -- Payout successful
        'payout_failed',      -- Payout failed, needs intervention
        'payout_retry',       -- Auto-retry scheduled
        'closed',             -- Cycle complete, advanced to next
        'skipped',            -- Cycle was skipped
...
-- =====================================================
-- CYCLE CONTRIBUTIONS TABLE
-- Individual contribution tracking per member per cycle
-- =====================================================
CREATE TABLE IF NOT EXISTS cycle_contributions (
    ...
    status TEXT NOT NULL DEFAULT 'pending' CHECK (status IN (
        ...
        'late',         -- Overdue but still accepting
        'missed',       -- Defaulted
        'excused',      -- Excused (admin action)
        'covered'       -- Covered by reserve or another member
    )),
    ...
    covered_by TEXT, -- 'reserve', 'member:{userId}', 'guarantor:{userId}'
...
-- =====================================================
-- CYCLE EVENTS TABLE
-- Audit trail for all cycle state changes
-- =====================================================
...
CREATE TABLE IF NOT EXISTS member_defaults (
    ...
    resolution_method TEXT, -- 'payment', 'reserve', 'guarantor', 'write_off'
...
-- =====================================================
-- CIRCLE COMPLETIONS TABLE
-- Record when circles complete
-- =====================================================
CREATE TABLE IF NOT EXISTS circle_completions (
    ...
    -- Success metrics
    on_time_rate DECIMAL(5, 4), -- % of on-time payments
    completion_rate DECIMAL(5, 4), -- % of expected contributions received
...
CREATE TABLE IF NOT EXISTS scheduled_notifications (
    ...
    condition_check TEXT, -- 'if_not_paid', 'always', etc.
...
-- =====================================================
-- CYCLE ENGINE RUNS TABLE
-- Track engine execution for monitoring
-- =====================================================
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
        ...
        ALTER TABLE circles ADD COLUMN current_cycle_id UUID REFERENCES circle_cycles(id);
    END IF;

    -- Incomplete contribution policy
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
...
-- Circle Cycles - members can view their circle's cycles
...
-- Cycle Contributions - users see own, circle leaders see all in circle
...
-- Cycle Events - members can view their circle's events
...
-- Circle Completions - members can view
...
-- Scheduled Notifications - users see own
...
-- Engine Runs - admin only (handled at app level)
...
-- Trigger to update cycle totals when contribution changes
CREATE OR REPLACE FUNCTION update_cycle_totals()
RETURNS TRIGGER AS $$
BEGIN
    -- Update the parent cycle's totals
...
CREATE TABLE IF NOT EXISTS reserve_funds (
    ...
    max_coverage_percent DECIMAL(5, 4) DEFAULT 0.20, -- 20% max coverage per default
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW(),
    UNIQUE(community_id)
);
...
CREATE TABLE IF NOT EXISTS reserve_fund_transactions (
    ...
    reference_type TEXT, -- 'cycle', 'default', 'admin'
    reference_id UUID,
    notes TEXT,
...
-- XnScore History (for tracking score changes over time)
...
-- Vouches Table (for vouch system)
CREATE TABLE IF NOT EXISTS vouches (
    ...
    vouch_amount DECIMAL(15, 2), -- Maximum amount voucher is willing to cover
    reason TEXT,
...
-- User Payment Methods (for payout destinations)
CREATE TABLE IF NOT EXISTS user_payment_methods (
    ...
    account_identifier TEXT NOT NULL, -- Phone number, account number, etc.
    account_name TEXT,
...
-- Reserve Funds - community leaders can view
...
-- Vouches - voucher and vouchee can view
