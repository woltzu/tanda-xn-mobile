...
-- ╚═══════════════════════════════════════════════════════════════════════════╝
-- Note: Views created without SECURITY DEFINER are SECURITY INVOKER by default
-- We need to drop and recreate each view without the SECURITY DEFINER clause
...
SELECT
    ...
    ps.total_amount_cents / 100.0 as amount_due,
    COALESCE(ps.paid_amount_cents, 0) / 100.0 as amount_paid,
    (ps.total_amount_cents - COALESCE(ps.paid_amount_cents, 0)) / 100.0 as remaining,
    ps.status
FROM loan_payment_schedule ps
JOIN loans l ON l.id = ps.loan_id
JOIN profiles p ON p.id = l.user_id
WHERE ps.status = 'overdue' AND l.status = 'active';

-- ── v_loan_interest_summary ──────────────────────────────────────────────────
...
SELECT
    ...
    COALESCE(SUM(lf.fee_cents), 0) / 100.0 as total_late_fees,
    l.status
FROM loans l
LEFT JOIN loan_interest_accruals ia ON ia.loan_id = l.id
LEFT JOIN loan_late_fees lf ON lf.loan_id = l.id AND lf.status = 'applied'
GROUP BY l.id;
...
SELECT
    lg.guarantor_id as user_id,
    p.full_name,
    COUNT(*) as total_guarantees,
    COUNT(*) FILTER (WHERE lg.status = 'active') as active_guarantees,
    COALESCE(SUM(lg.guarantee_amount_cents) FILTER (WHERE lg.status = 'active'), 0) / 100.0 as total_exposure
FROM loan_guarantees lg
JOIN profiles p ON p.id = lg.guarantor_id
GROUP BY lg.guarantor_id, p.full_name;
...
-- ══════════════════════════════════════════════════════════════════════════════
-- END OF MIGRATION 025: Security Fixes
-- ══════════════════════════════════════════════════════════════════════════════
