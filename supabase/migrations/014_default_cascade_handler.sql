-- Migration: 014_default_cascade_handler.sql
...
BEGIN
    FOR r IN
        ...
        FROM pg_proc
        WHERE proname IN ('check_repeat_offender', 'calculate_voucher_reliability', 'get_default_cascade_summary')
    LOOP
...
CREATE TABLE defaults (
    ...
    -- Status: 'unresolved', 'partial_recovery', 'fully_recovered', 'written_off', 'forgiven', 'disputed'
...
-- =====================================================
-- VOUCHER DEFAULT IMPACTS - Track voucher consequences
-- =====================================================
CREATE TABLE voucher_default_impacts (
    ...
    voucher_reliability_status TEXT, -- 'good', 'warning', 'poor', 'restricted'
...
-- =====================================================
-- CIRCLE DEFAULT RESOLUTIONS - How circle handles the shortfall
-- =====================================================
CREATE TABLE circle_default_resolutions (
    ...
    -- 'reduced_payout', 'reserve_full_cover', 'reserve_partial_cover',
    -- 'redistribution_full', 'redistribution_partial', 'hybrid'
    ...
    -- Redistribution details (if used)
    ...
    -- Status: 'pending', 'pending_redistribution', 'resolved', 'partial', 'failed'
...
-- =====================================================
-- RECOVERY PLANS - Path back to good standing
-- =====================================================
CREATE TABLE recovery_plans (
    ...
    payment_type TEXT NOT NULL, -- 'lump_sum', 'installments', 'pending_selection'
    ...
    -- Status: 'offered', 'accepted', 'active', 'completed', 'defaulted_again', 'cancelled'
...
-- =====================================================
-- RECOVERY PLAN INSTALLMENTS - Individual payment tracking
-- =====================================================
...
-- =====================================================
-- SUSPENSION REVIEWS - For repeat offenders
-- =====================================================
CREATE TABLE suspension_reviews (
    ...
    -- 'three_defaults_12_months', 'two_unresolved_defaults', 'five_total_defaults'
    ...
    -- 'pending', 'under_review', 'suspended', 'warning_issued', 'cleared'
...
-- =====================================================
-- MEMBER DEBTS - When removed after receiving payout
-- =====================================================
CREATE TABLE member_debts (
    ...
    amount DECIMAL(12,2) NOT NULL,
    reason TEXT NOT NULL,
    -- 'removal_after_payout', 'early_exit_after_payout'
...
-- =====================================================
-- CASCADE EVENTS - Comprehensive audit trail
-- =====================================================
CREATE TABLE cascade_events (
    ...
    -- 'cascade_started', 'xnscore_impact_defaulter', 'voucher_impact_applied',
    -- 'voucher_restricted', 'circle_resolution_started', 'reserve_used',
    -- 'redistribution_requested', 'redistribution_completed', 'payout_reduced',
    -- 'restriction_applied', 'community_health_updated', 'recovery_plan_offered',
    -- 'notification_sent', 'cascade_completed', 'cascade_error',
    -- 'other_circles_evaluated', 'removed_from_circle', 'suspension_review_triggered'

    -- Context
    target_user_id UUID REFERENCES profiles(id),
    target_type TEXT, -- 'defaulter', 'voucher', 'recipient', 'community', 'circle'
...
-- Add columns to circles for default handling policies
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
        ...
        -- 'warn_only', 'increase_scrutiny', 'remove_if_repeat', 'remove_immediate'
    END IF;
END $$;

-- Add columns to circle_members for default tracking
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
...
-- Add columns to profiles for default tracking
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
...
-- Add columns to vouches for tracking vouchee defaults
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
...
-- INDEXES
-- =====================================================
CREATE INDEX idx_def_user ON defaults(user_id);
CREATE INDEX idx_def_circle ON defaults(circle_id);
CREATE INDEX idx_def_community ON defaults(community_id);
CREATE INDEX idx_def_status ON defaults(default_status);
CREATE INDEX idx_def_cascade ON defaults(cascade_id);
CREATE INDEX idx_def_unresolved ON defaults(user_id, default_status)
    WHERE default_status = 'unresolved';

CREATE INDEX idx_vdi_voucher ON voucher_default_impacts(voucher_user_id);
CREATE INDEX idx_vdi_default ON voucher_default_impacts(default_id);
CREATE INDEX idx_vdi_cascade ON voucher_default_impacts(cascade_id);

CREATE INDEX idx_cdr_default ON circle_default_resolutions(default_id);
CREATE INDEX idx_cdr_circle ON circle_default_resolutions(circle_id);
CREATE INDEX idx_cdr_recipient ON circle_default_resolutions(recipient_user_id);

CREATE INDEX idx_rp_user ON recovery_plans(user_id);
CREATE INDEX idx_rp_default ON recovery_plans(default_id);
CREATE INDEX idx_rp_status ON recovery_plans(plan_status);

CREATE INDEX idx_rpi_plan ON recovery_plan_installments(recovery_plan_id);
CREATE INDEX idx_rpi_due ON recovery_plan_installments(due_date)
    WHERE installment_status = 'pending';

CREATE INDEX idx_sr_user ON suspension_reviews(user_id);
CREATE INDEX idx_sr_status ON suspension_reviews(review_status);

CREATE INDEX idx_md_user ON member_debts(user_id);
CREATE INDEX idx_md_status ON member_debts(debt_status);

CREATE INDEX idx_ce_cascade ON cascade_events(cascade_id);
CREATE INDEX idx_ce_default ON cascade_events(default_id);
CREATE INDEX idx_ce_type ON cascade_events(event_type);
...
-- Defaults - user sees own, leaders see circle defaults
CREATE POLICY "def_select_policy" ON defaults
    FOR SELECT USING (
        user_id = auth.uid() OR
        EXISTS (SELECT 1 FROM circle_members WHERE circle_id = defaults.circle_id AND user_id = auth.uid() AND role IN ('leader', 'admin'))
    );

-- Voucher impacts - voucher sees their own impacts
CREATE POLICY "vdi_select_policy" ON voucher_default_impacts
    FOR SELECT USING (
        voucher_user_id = auth.uid() OR
        defaulter_user_id = auth.uid()
    );

-- Circle resolutions - circle members can see
CREATE POLICY "cdr_select_policy" ON circle_default_resolutions
    FOR SELECT USING (
...
-- Recovery plans - user sees own
CREATE POLICY "rp_select_policy" ON recovery_plans
    FOR SELECT USING (user_id = auth.uid());

-- Recovery installments - user sees own plans
CREATE POLICY "rpi_select_policy" ON recovery_plan_installments
    FOR SELECT USING (
        EXISTS (SELECT 1 FROM recovery_plans WHERE id = recovery_plan_installments.recovery_plan_id AND user_id = auth.uid())
    );

-- Suspension reviews - user sees own
CREATE POLICY "sr_select_policy" ON suspension_reviews
    FOR SELECT USING (user_id = auth.uid());

-- Member debts - user sees own
CREATE POLICY "md_select_policy" ON member_debts
    FOR SELECT USING (user_id = auth.uid());

-- Cascade events - based on default visibility
CREATE POLICY "ce_select_policy" ON cascade_events
    FOR SELECT USING (
        EXISTS (
...
-- Calculate voucher reliability status
...
-- Trigger function for defaults timestamp
CREATE OR REPLACE FUNCTION update_defaults_timestamp()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = NOW();
    RETURN NEW;
