-- ══════════════════════════════════════════════════════════════════════════════
-- MIGRATION 024 FIX: Monthly Payment System
-- ══════════════════════════════════════════════════════════════════════════════
-- Fix for: ERROR 42P07: relation "idx_reminders_user" already exists
-- This migration drops and recreates all objects to handle partial runs
-- ══════════════════════════════════════════════════════════════════════════════

-- ┌─────────────────────────────────────────────────────────────────────────────┐
-- │ DROP EXISTING INDEXES (if they exist from partial run)                      │
-- └─────────────────────────────────────────────────────────────────────────────┘

DROP INDEX IF EXISTS idx_obligations_loan CASCADE;
DROP INDEX IF EXISTS idx_obligations_user CASCADE;
DROP INDEX IF EXISTS idx_obligations_due_date CASCADE;
DROP INDEX IF EXISTS idx_obligations_status CASCADE;
DROP INDEX IF EXISTS idx_obligations_upcoming CASCADE;
DROP INDEX IF EXISTS idx_autopay_loan CASCADE;
DROP INDEX IF EXISTS idx_autopay_user CASCADE;
DROP INDEX IF EXISTS idx_autopay_status CASCADE;
DROP INDEX IF EXISTS idx_autopay_next CASCADE;
DROP INDEX IF EXISTS idx_reminders_loan CASCADE;
DROP INDEX IF EXISTS idx_reminders_user CASCADE;
DROP INDEX IF EXISTS idx_reminders_scheduled CASCADE;
DROP INDEX IF EXISTS idx_reminders_obligation CASCADE;
...
DROP TRIGGER IF EXISTS tr_obligations_updated ON loan_payment_obligations CASCADE;
DROP TRIGGER IF EXISTS tr_autopay_updated ON loan_autopay_configs CASCADE;
DROP TRIGGER IF EXISTS tr_reminders_updated ON loan_payment_reminders CASCADE;

-- ┌─────────────────────────────────────────────────────────────────────────────┐
-- │ DROP EXISTING POLICIES                                                      │
-- └─────────────────────────────────────────────────────────────────────────────┘

DROP POLICY IF EXISTS "obligations_own_select" ON loan_payment_obligations;
DROP POLICY IF EXISTS "obligations_own_update" ON loan_payment_obligations;
DROP POLICY IF EXISTS "autopay_own_select" ON loan_autopay_configs;
DROP POLICY IF EXISTS "autopay_own_insert" ON loan_autopay_configs;
DROP POLICY IF EXISTS "autopay_own_update" ON loan_autopay_configs;
DROP POLICY IF EXISTS "reminders_own_select" ON loan_payment_reminders;
...
BEGIN
    ...
    INSERT INTO loan_payment_obligations (
        loan_id, user_id, obligation_number,
        period_start_date, period_end_date, due_date,
        estimated_payment_cents, principal_due_cents, interest_due_cents, total_due_cents,
        status
    ) VALUES (
        p_loan_id, v_loan.user_id, v_obligation_num,
        v_period_start, v_period_end, v_due_date,
        v_monthly_payment, v_principal_for_period, v_interest_for_period, v_monthly_payment,
        CASE WHEN v_due_date <= CURRENT_DATE THEN 'due'::obligation_status ELSE 'upcoming'::obligation_status END
    )
...
-- ┌─────────────────────────────────────────────────────────────────────────────┐
-- │ GENERATE ALL MONTHLY OBLIGATIONS                                            │
-- └─────────────────────────────────────────────────────────────────────────────┘
...
BEGIN
    ...
    LOOP
        ...
        v_title := format('Payment Due %s',
            CASE WHEN v_reminder_day = 0 THEN 'Today'
                 WHEN v_reminder_day = 1 THEN 'Tomorrow'
                 ELSE format('in %s days', v_reminder_day) END);
        ...
        LOOP
            INSERT INTO loan_payment_reminders (
                ...
                v_obligation.loan_id, v_obligation.user_id, p_obligation_id,
                v_reminder_type, v_channel, v_scheduled_time, v_reminder_day,
                v_title, v_message, v_obligation.total_due_cents, v_obligation.due_date,
...
BEGIN
    FOR v_reminder IN
        ...
        ORDER BY r.scheduled_for ASC LIMIT 100
    LOOP
        BEGIN
            IF v_reminder.obligation_status = 'paid' THEN
                UPDATE loan_payment_reminders SET status = 'cancelled', updated_at = now()
                WHERE id = v_reminder.id;
                v_processed := v_processed + 1;
                CONTINUE;
            END IF;

            UPDATE loan_payment_reminders SET status = 'sent', sent_at = now(), updated_at = now()
            WHERE id = v_reminder.id;
...
CREATE FUNCTION mark_reminder_sent(p_reminder_id UUID, p_notification_id TEXT DEFAULT NULL)
RETURNS BOOLEAN AS $$
BEGIN
    UPDATE loan_payment_reminders SET
        status = 'sent', sent_at = now(), notification_id = p_notification_id, updated_at = now()
...
BEGIN
    ...
    SELECT * INTO v_next_obligation FROM loan_payment_obligations
    WHERE loan_id = p_loan_id AND status IN ('upcoming', 'due')
    ...
    IF p_enabled THEN
        IF v_config IS NULL THEN
            INSERT INTO loan_autopay_configs (
                ...
                p_payment_method_id, p_fixed_amount_cents, 'active', v_next_scheduled
        ...
        ELSE
            UPDATE loan_autopay_configs SET
                ...
                status = 'active', next_scheduled_at = v_next_scheduled, updated_at = now()
            WHERE loan_id = p_loan_id RETURNING id INTO v_config_id;
        END IF;

        UPDATE loans SET
            autopay_enabled = TRUE, autopay_config_id = v_config_id,
            autopay_next_scheduled_at = v_next_scheduled, updated_at = now()
        WHERE id = p_loan_id;

        RETURN QUERY SELECT v_config_id, TRUE, p_autopay_type::autopay_type, v_next_scheduled, 'Autopay enabled successfully';
    ELSE
        IF v_config IS NOT NULL THEN
            UPDATE loan_autopay_configs SET status = 'disabled', next_scheduled_at = NULL, updated_at = now()
            WHERE loan_id = p_loan_id;
        END IF;

        UPDATE loans SET autopay_enabled = FALSE, autopay_next_scheduled_at = NULL, updated_at = now()
        WHERE id = p_loan_id;

        RETURN QUERY SELECT v_config.id, FALSE, v_config.autopay_type, NULL::TIMESTAMPTZ, 'Autopay disabled';
    END IF;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
BEGIN
    ...
    IF v_config.status != 'active' THEN
        RETURN QUERY SELECT FALSE, NULL::UUID, 0, NULL::UUID, format('Autopay is %s', v_config.status);
        RETURN;
    END IF;

    SELECT * INTO v_loan FROM loans WHERE id = v_config.loan_id;
    IF v_loan.status != 'active' THEN
        RETURN QUERY SELECT FALSE, NULL::UUID, 0, NULL::UUID, format('Loan is %s', v_loan.status);
        RETURN;
    END IF;

    SELECT * INTO v_obligation FROM loan_payment_obligations
    WHERE loan_id = v_config.loan_id AND status IN ('due', 'upcoming', 'overdue')
    ...
    SELECT * INTO v_payment_result FROM apply_payment_to_loan(
        v_config.loan_id, v_amount, 'autopay', v_config.payment_method_id::TEXT, NULL
    );

    UPDATE loan_payment_obligations SET
        status = CASE WHEN v_payment_result.remaining_principal_cents <= 0 THEN 'paid'::obligation_status ELSE 'partial'::obligation_status END,
        ...
        paid_at = now(), paid_via = 'autopay', updated_at = now()
    WHERE id = v_obligation.id;

    UPDATE loan_autopay_configs SET
        last_executed_at = now(), last_execution_status = 'success',
        total_payments_made = total_payments_made + 1,
        total_amount_paid_cents = total_amount_paid_cents + v_amount,
        consecutive_failures = 0, current_retry_count = 0, updated_at = now()
    WHERE id = p_config_id;

    UPDATE loan_autopay_configs SET next_scheduled_at = (
        SELECT due_date::TIMESTAMPTZ + INTERVAL '9 hours' FROM loan_payment_obligations
        WHERE loan_id = v_config.loan_id AND status IN ('upcoming')
        ORDER BY due_date ASC LIMIT 1
    ) WHERE id = p_config_id;

    IF v_obligation.due_date >= CURRENT_DATE THEN
        UPDATE loan_payment_obligations SET
            xnscore_event_triggered = TRUE, xnscore_adjustment = 3
        WHERE id = v_obligation.id;
    END IF;

    RETURN QUERY SELECT TRUE, v_payment_result.payment_id, v_amount, v_obligation.id, NULL::TEXT;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
BEGIN
    FOR v_config IN
        SELECT ac.* FROM loan_autopay_configs ac
        JOIN loans l ON l.id = ac.loan_id
        WHERE ac.status = 'active' AND l.status = 'active' AND ac.next_scheduled_at <= now()
        ORDER BY ac.next_scheduled_at ASC LIMIT 50
    LOOP
        BEGIN
            ...
            ELSE
                ...
                UPDATE loan_autopay_configs SET
                    ...
                    status = CASE WHEN consecutive_failures + 1 >= max_retries THEN 'failed'::autopay_status ELSE status END,
                    updated_at = now()
...
BEGIN
    ...
    IF v_result.success THEN
        RETURN QUERY SELECT TRUE, v_result.payment_id, v_result.amount_cents, 'Retry successful';
    ELSE
...
BEGIN
    RETURN QUERY
    SELECT o.id, o.obligation_number, o.due_date, (o.due_date - CURRENT_DATE)::INTEGER,
    ...
    WHERE o.loan_id = p_loan_id AND o.status IN ('upcoming', 'due', 'overdue', 'partial')
...
RETURNS TABLE (
    obligation_id UUID, obligation_number INTEGER, period_start DATE, period_end DATE,
    due_date DATE, total_due_cents INTEGER, total_paid_cents INTEGER, remaining_cents INTEGER,
    status obligation_status, is_overdue BOOLEAN, days_overdue INTEGER,
    late_fee_cents INTEGER, paid_at TIMESTAMPTZ, paid_via TEXT
) AS $$
BEGIN
    RETURN QUERY
    SELECT o.id, o.obligation_number, o.period_start_date, o.period_end_date, o.due_date,
    ...
    FROM loan_payment_obligations o
    WHERE o.loan_id = p_loan_id AND (p_status IS NULL OR o.status = ANY(p_status))
...
BEGIN
    FOR v_obligation IN
        SELECT o.*, l.loan_product_id FROM loan_payment_obligations o
    ...
    LOOP
        BEGIN
            ...
            UPDATE loan_payment_obligations SET status = 'overdue', days_overdue = v_days_overdue, updated_at = now()
            WHERE id = v_obligation.id;
            v_updated := v_updated + 1;

            IF v_days_overdue > 5 AND NOT v_obligation.late_fee_applied THEN
                SELECT * INTO v_fee_result FROM apply_late_fee_to_loan(v_obligation.loan_id, v_obligation.id, v_days_overdue);
                IF v_fee_result.applied THEN
                    UPDATE loan_payment_obligations SET
                        late_fee_applied = TRUE, late_fee_id = v_fee_result.fee_id, late_fee_cents = v_fee_result.fee_cents,
                        fees_due_cents = fees_due_cents + v_fee_result.fee_cents,
                        total_due_cents = total_due_cents + v_fee_result.fee_cents, updated_at = now()
                    WHERE id = v_obligation.id;
                    v_fees := v_fees + 1;
                END IF;
            END IF;
            ...
            IF NOT v_obligation.xnscore_event_triggered AND v_days_overdue >= 1 THEN
                UPDATE loan_payment_obligations SET xnscore_event_triggered = TRUE, xnscore_adjustment = -5
                WHERE id = v_obligation.id;
            ...
            IF v_days_overdue IN (1, 3, 7, 14, 30) THEN
                INSERT INTO loan_payment_reminders (
                    loan_id, user_id, obligation_id, reminder_type, channel, scheduled_for, days_before_due,
                    title, message, amount_due_cents, due_date, status
                ) VALUES (
                    v_obligation.loan_id, v_obligation.user_id, v_obligation.id,
                    CASE WHEN v_days_overdue >= 14 THEN 'final_warning'::reminder_type ELSE 'overdue'::reminder_type END,
                    'push'::reminder_channel, now(), -v_days_overdue,
                    format('Payment %s Days Overdue', v_days_overdue),
                    format('Your payment of $%s is %s days overdue. Pay now to avoid additional fees.',
                        (v_obligation.total_due_cents / 100.0)::NUMERIC(10,2), v_days_overdue),
                    v_obligation.total_due_cents, v_obligation.due_date, 'scheduled'::reminder_status
                );
            END IF;
...
SELECT o.id as obligation_id, o.loan_id, o.user_id, p.full_name, o.obligation_number, o.due_date,
    ...
    o.total_due_cents / 100.0 as total_due, o.total_paid_cents / 100.0 as total_paid,
...
LEFT JOIN LATERAL (
    SELECT * FROM loan_payment_obligations WHERE loan_id = ac.loan_id AND status IN ('upcoming', 'due')
    ORDER BY due_date ASC LIMIT 1
) o ON TRUE
...
CREATE TRIGGER tr_obligations_updated BEFORE UPDATE ON loan_payment_obligations
...
CREATE TRIGGER tr_autopay_updated BEFORE UPDATE ON loan_autopay_configs
...
CREATE TRIGGER tr_reminders_updated BEFORE UPDATE ON loan_payment_reminders
