-- ============================================================================
-- TANDAXN COMMUNITY SYSTEM - MIGRATION 005 (FIXED)
-- ============================================================================
...
-- ============================================================================
-- STEP 2: Create COMMUNITIES table (without self-reference first)
-- ============================================================================
CREATE TABLE IF NOT EXISTS public.communities (
    ...
    icon TEXT DEFAULT '👥',
    image_url TEXT,
    parent_community_id UUID, -- Will add FK later
...
BEGIN
    ...
    ADD CONSTRAINT communities_parent_fkey
    FOREIGN KEY (parent_community_id) REFERENCES public.communities(id) ON DELETE SET NULL;
EXCEPTION WHEN duplicate_object THEN NULL;
END$$;

-- Add FK for circles -> communities
DO $$
BEGIN
...
CREATE TABLE IF NOT EXISTS public.vouch_events (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    vouch_id UUID REFERENCES public.member_vouches(id) ON DELETE CASCADE,
...
-- MEMBER VOUCHES
DROP POLICY IF EXISTS "vouches_select" ON public.member_vouches;
DROP POLICY IF EXISTS "vouches_insert" ON public.member_vouches;
DROP POLICY IF EXISTS "vouches_update" ON public.member_vouches;
...
-- VOUCH EVENTS
DROP POLICY IF EXISTS "vouch_events_select" ON public.vouch_events;

CREATE POLICY "vouch_events_select" ON public.vouch_events FOR SELECT
USING (true); -- Simplified for now

-- COMMUNITY INVITATIONS
DROP POLICY IF EXISTS "invitations_select" ON public.community_invitations;
DROP POLICY IF EXISTS "invitations_insert" ON public.community_invitations;
...
-- DEFAULTS
DROP POLICY IF EXISTS "defaults_select" ON public.defaults;

CREATE POLICY "defaults_select" ON public.defaults FOR SELECT
USING (user_id = auth.uid());

-- GRACE PERIODS
DROP POLICY IF EXISTS "grace_periods_select" ON public.default_grace_periods;
...
USING (reporter_user_id = auth.uid() OR against_user_id = auth.uid() OR assigned_to = auth.uid());
...
DROP POLICY IF EXISTS "activities_select" ON public.community_activities;
...
-- ELDER APPLICATIONS
DROP POLICY IF EXISTS "elder_applications_select" ON public.elder_applications;
DROP POLICY IF EXISTS "elder_applications_insert" ON public.elder_applications;
...
-- ELDER VOTE RECORDS
DROP POLICY IF EXISTS "vote_records_select" ON public.elder_vote_records;
DROP POLICY IF EXISTS "vote_records_insert" ON public.elder_vote_records;
